...
const GATE_EFFECT_SYMBOLS: Record<GateOperation, string> = {
  add: '+',
  subtract: '−',
  multiply: '×',
  divide: '÷',
...
/** Displays the operation applied to the crowd, avoiding equation-style ambiguity. */
export function formatGateEffect(gate: GateOption): string {
  const operand = Math.max(1, Math.round(gate.operand))
  return `${GATE_EFFECT_SYMBOLS[gate.operation]}${operand}`
}

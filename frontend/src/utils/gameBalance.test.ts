...
import { applyGateOption, computeBestRoute, formatGateEffect, generateGatePairs } from './mathGates'
...
describe('math gates', () => {
  it('labels gates by their crowd effect instead of as an ambiguous equation', () => {
    expect(formatGateEffect({ expr: '1 × 2', operation: 'multiply', operand: 2 })).toBe('×2')
    expect(formatGateEffect({ expr: '1 + 1', operation: 'add', operand: 2 })).toBe('+2')
    expect(formatGateEffect({ expr: '4 − 2', operation: 'subtract', operand: 2 })).toBe('−2')
    expect(formatGateEffect({ expr: '4 ÷ 2', operation: 'divide', operand: 2 })).toBe('÷2')
  })

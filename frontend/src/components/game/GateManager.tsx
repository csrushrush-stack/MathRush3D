...
import { applyGateOption, formatGateEffect, type GateOption, type GatePairData } from '../../utils/mathGates'
...
function createGateTexture(option: GateOption, accent: string) {
  ...
  context.fillText(formatGateEffect(option), canvas.width / 2, canvas.height / 2)
  ...
  useFrame(() => {
    ...
    for (const pair of sourcePairs) {
      ...
      state.recordGateChoice({
        gateIndex: pair.id,
        worldZ: pair.worldZ,
        leftExpression: formatGateEffect(pair.left),
        rightExpression: formatGateEffect(pair.right),
        chosenSide,

...
import { fetchSkinCatalog, purchaseSkin } from '../../services/api'
...
export function SkinsModal({ onClose }: SkinsModalProps) {
  ...
  const [skins, setSkins] = useState<readonly SkinDefinition[]>(SKINS)

  useEffect(() => {
    let active = true
    void fetchSkinCatalog()
      .then((catalog) => { if (active && catalog.length) setSkins(catalog) })
      .catch(() => undefined)
    return () => { active = false }
  }, [])
